...
gps.in_cahb = and(and(gps.lons>=lonbounds(1),gps.lons<=lonbounds(2)),and(gps.lats>=latbounds(1),gps.lats<=latbounds(2)));
kreemer14.in_cahb = and(and(kreemer14.lons>=lonbounds(1),kreemer14.lons<=lonbounds(2)),and(kreemer14.lats>=latbounds(1),kreemer14.lats<=latbounds(2)));
gps.toremove = gps.in_cahb==0;
kreemer14.toremove = kreemer14.in_cahb==0;

gps.lons(gps.toremove) = [];
gps.lats(gps.toremove) = [];
gps.veast_itrf(gps.toremove) = [];
gps.vnorth_itrf(gps.toremove) = [];
gps.seast(gps.toremove) = [];
gps.snorth(gps.toremove) = [];
gps.vup(gps.toremove) = [];
gps.sup(gps.toremove) = [];
gps.occtime(gps.toremove) = [];
gps.coven(gps.toremove) = [];
gps.nocc(gps.toremove) = [];
gps.names(gps.toremove) = [];

kreemer14.lons(kreemer14.toremove) = [];
kreemer14.lats(kreemer14.toremove) = [];
kreemer14.veast_itrf(kreemer14.toremove) = [];
kreemer14.vnorth_itrf(kreemer14.toremove) = [];
kreemer14.seast(kreemer14.toremove) = [];
kreemer14.snorth(kreemer14.toremove) = [];
kreemer14.occtime(kreemer14.toremove) = [];
kreemer14.coven(kreemer14.toremove) = [];
kreemer14.nocc(kreemer14.toremove) = [];
kreemer14.names(kreemer14.toremove) = [];
...
print(['tianshan_all_6July.png'],'-dpng','-r300');
%return
...
gps_c.toremove_h = or(gps_c.horizsig>tolh,or(gps_c.occtime<minocctime,gps_c.nocc<minocc));
gps_c.toremove_v = or(gps_c.sup>tolv,or(gps_c.occtime<minocctime,gps_c.nocc<minocc));
%return
kreemer14_c.horizsig = sqrt(kreemer14_c.seast.^2 + kreemer14_c.snorth.^2);
kreemer14_c.toremove = or(kreemer14_c.horizsig>tolh,or(kreemer14_c.occtime<minocctime,kreemer14_c.nocc<minocc));
...
gps_c.veast_eu(gps_c.toremove) = [];
gps_c.vnorth_eu(gps_c.toremove) = [];
...
kreemer14_c.veast_eu(kreemer14_c.toremove) = [];
kreemer14_c.vnorth_eu(kreemer14_c.toremove) = [];
...
%%
% gps_c.names_orig = gps_c.names;
% for gpsindex = 1:length(gps_c.lons)
%     gps_c.names{gpsindex} = gps_c.names{gpsindex}(1:4);
% end
...
%text(gps_c.lons,gps_c.lats,lower(gps_c.names))
...
%return
print(['tianshan_tolh' num2str(tolh*1e3) '_minocctime' num2str(minocctime) '_all_6July.png'],'-dpng','-r300');
%%
%return
%%
gps_m = gps_c;
gps_m.names_orig = gps_m.names;
for gpsindex = 1:length(gps_m.lons)
    gps_m.names{gpsindex} = gps_m.names{gpsindex}(1:4);
end
gps_m.names = lower(gps_m.names);
...
for gpsindex = 1:length(gps_m.lons)
    ...
    samestations = find(and(samestations,stadists<=20));
    if sum(stadists(samestations)>20)>0
        pause
    end
%     if strcmp(gps_m.names(gpsindex),'i389')
%         pause
%     end

    for powerindex = 10:-1:1
...
%output_plot2D = or(gps_m.output_honly,gps_m.output_hv);
%gpsplot = quiver(gps_m.lons(gps_m.output_honly),gps_m.lats(gps_m.output_honly),sc/cosd(37.5)*gps_m.veast_eu(gps_m.output_honly),sc*gps_m.vnorth_eu(gps_m.output_honly),0,'color','k','linewidth',0.75);
...
print(['tianshan_tolh' num2str(tolh*1e3) '_minocctime' num2str(minocctime) '_group_6July.png'],'-dpng','-r300');
...
gps_m.names_orig = gps_m.names_orig(sortorder);
...
gps_m.veast_eu = gps_m.veast_eu(sortorder);
gps_m.vnorth_eu = gps_m.vnorth_eu(sortorder);
...
%gps_m.horizsig = gps_m.numcloseby(sortorder);
...
for gpsindex = 1:length(gps_m.lons)
    if 1==touse(gpsindex)
        ...
        gps_o.veast_eu(end+1) = mean(gps_m.veast_eu(stationscloseby_withhoriz));
        ...
        gps_o.vnorth_eu(end+1) = mean(gps_m.vnorth_eu(stationscloseby_withhoriz));
...
gps_o.veast_eu = gps_o.veast_eu';
gps_o.vnorth_eu = gps_o.vnorth_eu';
gps_o.vup = gps_o.vup';
...
%%
gps_o.output_honly = and(isnan(gps_o.sup),~isnan(gps_o.snorth));
gps_o.output_hv = and(~isnan(gps_o.sup),~isnan(gps_o.snorth));
gps_o.output_vonly = and(~isnan(gps_o.sup),isnan(gps_o.snorth));

figure(4); clf; hold on; box on;
sc = 3e1;
output_plotvert = or(gps_o.output_hv,gps_o.output_vonly);
scatter(gps_o.lons(output_plotvert),gps_o.lats(output_plotvert),75,1e3*gps_o.vup(output_plotvert),'filled','markeredgecolor','none')
plot(allcoastlines(:,1),allcoastlines(:,2),'color',[0 0.5 0.75],'linewidth',0.5)
plot(borderlons,borderlats,'color',[0.5 0.5 0.5],'linewidth',0.5)
plot(gem_plotready(:,1),gem_plotready(:,2),'color',[1 0.75 0.75],'linewidth',0.5);

output_plot2D = or(gps_o.output_honly,gps_o.output_hv);
%gpsplot = quiver(gps_o.lons(gps_o.output_honly),gps_o.lats(gps_o.output_honly),sc/cosd(37.5)*gps_o.veast_eu(gps_o.output_honly),sc*gps_o.vnorth_eu(gps_o.output_honly),0,'color','k','linewidth',0.75);
...
%text(gps_o.lons(gps_o.output_honly),gps_o.lats(gps_o.output_honly),lower(gps_o.names(gps_o.output_honly)))
...
print(['tianshan_tolh' num2str(tolh*1e3) '_minocctime' num2str(minocctime) '_meanwithin' num2str(thresholddist) '_6July.png'],'-dpng','-r300');
%return

%%
%%
ffile = fopen(['~/Google Drive/AHB/Material/GPS/' inputregion '_tol' num2str(tolh*1e3) '_minocc' num2str(minocctime) '_dist' num2str(thresholddist) '_edges_2D_6july2023.dat'],'wt');
h_export = find(gps_o.output_honly);
...
ffile = fopen(['~/Google Drive/AHB/Material/GPS/' inputregion '_tol' num2str(tolh*1e3) '_minocc' num2str(minocctime) '_dist' num2str(thresholddist) '_edges_2D_fake3D_6july2023.dat'],'wt');
h_export = find(gps_o.output_honly);
for gpsindex = 1:length(h_export)
...
v_export = find(gps_o.output_vonly);
...
fprintf(ffile,'%.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %s\n',...
    ...
    0,...
    0,...
    gps_o.vup(v_export(gpsindex)),...
    1e3,...
    1e3,...
    gps_o.sup(v_export(gpsindex)),...
    0,...
    0,...
    0,...
...
ffile = fopen(['~/Google Drive/AHB/Material/GPS/' inputregion '_tol' num2str(tolv*1e3) '_minocc' num2str(minocctime) '_dist' num2str(thresholddist) '_edges_3D_6july2023.dat'],'wt');
hv_export = find(gps_o.output_hv);
for gpsindex = 1:length(hv_export)
fprintf(ffile,'%.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %s\n',...
    gps_o.lons(hv_export(gpsindex)),...
    gps_o.lats(hv_export(gpsindex)),...
    gps_o.veast_eu(hv_export(gpsindex))*1e3,...
    gps_o.vnorth_eu(hv_export(gpsindex))*1e3,...
    gps_o.vup(hv_export(gpsindex))*1e3,...
    gps_o.seast(hv_export(gpsindex))*1e3,...
    gps_o.snorth(hv_export(gpsindex))*1e3,...
    gps_o.sup(hv_export(gpsindex))*1e3,...
    gps_o.coven(hv_export(gpsindex)),...
    0*gps_o.snorth(hv_export(gpsindex)),...
    0*gps_o.sup(hv_export(gpsindex)),...
    gps_o.names{hv_export(gpsindex)});
end
fclose(ffile);

save(['gpsoutput_' inputregion '_' num2str(tolh*1e3) '_' num2str(minocctime) '_' num2str(thresholddist) '_2D_3D_itrfeu_6july2023.mat'],'gps','gps_c','gps_m','gps_o')